...
from firm_foundations.data import load_dataset_with_metadata, ThePile
from firm_foundations.features import FeatureSet, BoW, TFIDF, Word2Vec, BertFeatureComputer, LOSS, LossRatio, ZLIB, LossZLIB, MinK, MinKXX, ALoRa, GradNorm, MoPe, ReCaLL, DCPDD, ModelStealing, JL, DetectGPT, Quantile
from firm_foundations.classifiers import Threshold, DecisionTree, RandomForest, GradientBoosting, LogReg, NeuralNet
from firm_foundations.utils.plot_utils import plot_ROC, plot_histogram
from firm_foundations.utils.log_utils import get_my_logger, get_git_hash, clean_filename, mem_stats
...
def add_dataset_arguments(parser: jsonargparse.ArgumentParser):
    ...
    parser.add_argument("--dataset.num_train_samples", type=int, required=True, default=0, help="Number of train samples to take from the dataset - set to 1 for unsupervised MIA")
    parser.add_argument("--dataset.num_val_samples", type=int, required=True, default=0, help="Number of val samples to take from the dataset.")
...
def add_wandb_arguments(parser: jsonargparse.ArgumentParser):
    ...
    parser.add_argument("--wandb.project", type=str, required=False, default="firm_foundations", help="Wandb project name")
...
def main():
    ...
    if args.features.compute:
        ...
        try:
            dataset = load_dataset_with_metadata(args.dataset.name)
        except:
            raise ValueError(f"Unrecognized dataset: {args.dataset.name}")

        config = AutoConfig.from_pretrained(args.model.name, revision=args.model.revision, cache_dir=args.model.cache_dir)
        tokenizer = AutoTokenizer.from_pretrained(args.model.name, revision=args.model.revision, cache_dir=args.model.cache_dir)
